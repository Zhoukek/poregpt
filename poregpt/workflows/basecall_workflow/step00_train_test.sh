...
export PYTHONPATH=/mnt/zzbnew/rnamodel/zhoukexuan/poregpt/poregpt/dcbasecaller:/mnt/zzbnew/rnamodel/zhoukexuan/poregpt/poregpt/workflows/olmo_workflow/OLMo:/mnt/zzbnew/rnamodel/zhoukexuan/poregpt:${PYTHONPATH:-}
...
wandb_run_name="dna32g_${model_name}_unfreeze${unfreeze_last_n_layers}_bsz${batch_size}_baseline"
...
nohup torchrun --nproc_per_node="${nproc_per_node}" --nnodes=1 \
  -m scripts.basecaller_v8_0420.train_ddp_multifolder \
  ...
  --ddp_backend nccl \
  --resume_ckpt /mnt/zzbnew/rnamodel/shenhaojie/signalDNAmodel/test-haojieshen-model-type26-cnn_type13_teacher_model_distill0.1_VQ_64k_lemon/dna32g_test-haojieshen-model-type26-cnn_type13_teacher_model_distill0.1_VQ_64k_lemon_unfreeze4_bsz32_baseline/ckpt_best.pt \
  --save_best \
  --use_wandb \
