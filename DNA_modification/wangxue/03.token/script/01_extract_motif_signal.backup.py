...
def main(args):
    ...
    print("[NOTE] Step1 outputs raw window signal (no downsample); tokenizer will handle stride/compression later.")
...
def parse_args():
    ...
    ap.add_argument("--pattern", default="ATAACAGGT", help="motif sequence to search in seq")
    ap.add_argument("--target_strand", type=int, default=0, help="only keep records with this align_strand")
    ...
    ap.add_argument("--normalize_mode", default="mad", choices=["apple","stone", "none","lemon"], help="signal normalization mode")
    ap.add_argument("--write_window_signal", action="store_true", help="write windowed signal into output JSONL")
    ap.add_argument("--write_moves", action="store_true", help="write full moves into output JSONL")
    ap.add_argument("--write_seq", action="store_true", help="write full seq into output JSONL")
