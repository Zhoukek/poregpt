...
class NanoporeVQEModel_V25(nn.Module):
    ...
    def __init__(
        ...
        # 你要使用的模型名称
        self.teacher_model_dir = "/mnt/zzbnew/rnamodel/model/bonito/dna_basic_0121"

        # 指定设备并加载模型
        self.teacher_device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.teacher_model = load_model(self.teacher_model_dir, device=self.teacher_device)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Dict]:
        ...
        # print(f"Distill loss value: {distill_loss.item()}")
        # print(f"Reconstruction loss value: {recon.shape}")
        # print(f"all_loss value: {all_loss.shape}")

        return recon, indices, loss, loss_breakdown, distill_loss
