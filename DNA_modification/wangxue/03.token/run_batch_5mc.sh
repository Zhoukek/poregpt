...
# Basic settings
# =========================
PATTERN_FILE="./data/5mc_mask_1.txt"
...
TOKENIZER_CKPT="/mnt/zzbnew/rnamodel/model/signalDNAmodel/${MODEL_NAME}/encoder"
MODEL_PATH="/mnt/zzbnew/rnamodel/model/signalDNAmodel/${MODEL_NAME}/base"
...
#     --y_lim -3 3
  ...
  #   --jsonl_path "${JSONL_A}" \
  #   --h5_path "${H5_A}" \
  #   --left_flank ATCGGCGCACCTGCCGGAAAGGCCG \
  #   --right_flank GATCCGGAACAGTTTTTCTGCTTCC \
  #   --pair_mode nearest \
  #   --stride "${STRIDE}" \
  #   --pad_samples "${PAD_SAMPLES}" \
  #   --batch_size 100 \
...
normal="mongo"
  ...
  #### Step2: tokenize
  python3 script/02_tokenize_base_tokens_layer1.py \
...
#### Step1: 提取目标序列信号
  ...
  #   --batch_size 100 \
  #   --out_dir "${OUT_A}/png" \
  #   --out_txt "${OUT_A}/signal_${normal}_between_flanks.txt" \
  #   --out_jsonl "${OUT_A}/signal_${normal}_between_flanks.jsonl" \
  #   --normalize_mode "${normal}" \
  #   --write_window_signal \
  #   --write_debug_meta \
  #   --y_lim_mode manual \
  ...
  #   --batch_size 100 \
  #   --out_dir "${OUT_B}/png" \
  #   --out_txt "${OUT_B}/signal_${normal}_between_flanks.txt" \
  #   --out_jsonl "${OUT_B}/signal_${normal}_between_flanks.jsonl" \
  #   --normalize_mode "${normal}" \
  #   --write_window_signal \
  #   --write_debug_meta \
  #   --y_lim_mode manual \
  ...
  # #### Step2: tokenize
  # python3 script/02_tokenize_base_tokens_layer1.py \
  #   --input_jsonl "${OUT_A}/signal_${normal}_between_flanks.jsonl" \
  #   --output_jsonl "${OUT_A}/base_tokens_between_flanks.jsonl" \
  #   --model_ckpt "${TOKENIZER_CKPT}" \
  #   --device "${DEVICE}" \
  #   --map_mode stride --token_stride "${TOKEN_STRIDE}" --ensure_non_empty

  # python3 script/02_tokenize_base_tokens_layer1.py \
  #   --input_jsonl "${OUT_B}/signal_${normal}_between_flanks.jsonl" \
  #   --output_jsonl "${OUT_B}/base_tokens_between_flanks.jsonl" \
  #   --model_ckpt "${TOKENIZER_CKPT}" \
  #   --device "${DEVICE}" \
  #   --map_mode stride --token_stride "${TOKEN_STRIDE}" --ensure_non_empty
  ...
  # python3 script/03_embed_per_base.py \
  #   --model_path "${MODEL_PATH}" \
  #   --input_jsonl "${OUT_A}/base_tokens_between_flanks.jsonl" \
  #   --out_dir "${OUT_A}/base_embedding_between_flanks" \
  #   --label "${PATTERN}" \
  #   --pool mean

  # python3 script/03_embed_per_base.py \
  #   --model_path "${MODEL_PATH}" \
  #   --input_jsonl "${OUT_B}/base_tokens_between_flanks.jsonl" \
  #   --out_dir "${OUT_B}/base_embedding_between_flanks" \
  #   --label "${PATTERN}" \
  #   --pool mean
