...
from transformers import Olmo2Config, Olmo2ForCausalLM, CodeAwareOlmo2ForCausalLM
...
def write_model(
    ...
    for layer_i in range(n_layers):
        ...
        state_dict = {
            ...
            f"model.layers.{layer_i}.self_attn.code_q_proj.weight": loaded[
                f"transformer.blocks.{layer_i}.code_q_proj.weight"
            ],
            f"model.layers.{layer_i}.self_attn.code_k_proj.weight": loaded[
                f"transformer.blocks.{layer_i}.code_k_proj.weight"
            ],
    ...
    # 读取codebook
    codebook_path = "/mnt/zzbnew/rnamodel/shenhaojie/signalDNAmodel/test-haojieshen-model-type26-cnn_type13_teacher_model_distill0.1_VQ_64k_lemon/encoder"
    tokenizer = VQETokenizer(model_ckpt=codebook_path)
    codebook = tokenizer._get_codebook_embed()
    ...
    model = CodeAwareOlmo2ForCausalLM.from_pretrained(tmp_model_path, torch_dtype=torch.float32, low_cpu_mem_usage=True)
