...
def validate_pattern_atgcn(pattern: str):
    ...
    if bad:
        raise ValueError(
            f"Unsupported bases: {bad}. Only A/T/G/C/N are supported."
        )
...
def build_base_sample_spans_rel(
    ...
    """
    Returns:
      base_sample_spans_rel: List[[rel0, rel1]] length=span_len_bases
      base_abs_spans:        List[[abs0, abs1]] length=span_len_bases
      ok_any:                bool
    """
    ...
    for pos in range(span_len_bases):
...
def reverse_spans_rel(base_sample_spans_rel, win_len):
    """
    Reverse local spans so that reversed signal still aligns with the
    original forward-style orientation.
    """
    rev = []
...
# flank pairing
# =========================
def pair_left_right_occurrences(seq_norm, left_flank, right_flank, mode="nearest"):
    """
    Return list of tuples:
        (left_start, left_end, right_start, right_end, middle_start, middle_end)

    mode:
      - nearest: for each left hit, choose the nearest right hit after it
      - all:     output all valid pairs
      - outer:   first left + last right after it
    """
    left_hits = find_all_occurrences_atgcn(seq_norm, left_flank)
    right_hits = find_all_occurrences_atgcn(seq_norm, right_flank)
    ...
    right_len = len(right_flank)

    if mode == "nearest":
        for l0 in left_hits:
            l1 = l0 + left_len
            right_after = [r0 for r0 in right_hits if r0 >= l1]
            if not right_after:
                continue
            r0 = right_after[0]
            r1 = r0 + right_len
            pairs.append((l0, l1, r0, r1, l1, r0))

    elif mode == "all":
        for l0 in left_hits:
            l1 = l0 + left_len
            for r0 in right_hits:
                if r0 >= l1:
                    r1 = r0 + right_len
                    pairs.append((l0, l1, r0, r1, l1, r0))
...
# Plot batch
# =========================
def save_batch_png_subplots(batch, batch_idx, out_dir, args):
    n = len(batch)
    if n == 0:
        return None

    os.makedirs(out_dir, exist_ok=True)
    out_png = os.path.join(out_dir, f"between_flanks_batch_{batch_idx:04d}.png")
    ...
    for ax, item in zip(axes, batch):
        ...
        # draw per-base spans for middle region
        max_label_bases = args.max_bases_to_label
        for pos in range(min(len(base_spans_rel), max_label_bases)):
            ...
            if pos in set(args.mark_base_pos or []):
                lw = 1.4
                alpha = 0.9
            ...
            if pos < len(middle_seq):
                mid = (a + b) / 2.0
                ax.text(
                    mid, 0.97, middle_seq[pos],
        ...
        ax.set_title(
            f"{rid} | strand={strand_name} | middle_len={len(middle_seq)}",
            fontsize=9,
            loc="left"
        )
    ...
    axes[-1].set_xlabel("window sample index (orientation unified if enabled)", fontsize=10)
    fig.suptitle(
        f"extract between flanks | batch={batch_idx:04d} | n={n} "
        f"| norm={args.normalize_mode} | plot_ds={args.downsample_plot} "
        f"| ylim={args.y_lim_mode}",
        fontsize=12,
        y=0.995
    )
...
# normalization
# =========================
def normalize_signal(signal_raw, mode):
    if mode == "apple":
...
def main(args):
    ...
    if len(args.left_flank) == 0 or len(args.right_flank) == 0:
        raise ValueError("--left_flank and --right_flank must not be empty")

    left_norm = normalize_seq_atgcn(args.left_flank)
    right_norm = normalize_seq_atgcn(args.right_flank)
    validate_pattern_atgcn(left_norm)
    validate_pattern_atgcn(right_norm)

    left_rc = reverse_complement_atgcn(left_norm)
    right_rc = reverse_complement_atgcn(right_norm)
    ...
    txt_out.write(
        "read_id\tstrand\tleft_start\tleft_end\tright_start\tright_end\tmiddle_start\tmiddle_end\tmiddle_seq_len\tmiddle_signal_start\tmiddle_signal_end\twin_signal_start\twin_signal_end\tleft_query\tright_query\tleft_matched\tright_matched\n"
    )
    ...
    with h5py.File(args.h5_path, "r") as h5, open(args.jsonl_path, "r", encoding="utf-8") as f:
        ...
        for line in f:
            ...
            if strand == 0:
                left_to_search = left_norm
                right_to_search = right_norm
                is_reverse = False
            else:
                # reverse strand sequence should be searched by reverse-complemented flanks
                left_to_search = left_rc
                right_to_search = right_rc
                is_reverse = True

            pairs = pair_left_right_occurrences(
                seq_norm=seq_norm,
                left_flank=left_to_search,
                right_flank=right_to_search,
                mode=args.pair_mode,
            )
            if not pairs:
                no_valid_pair_cnt += 1
            ...
            signal = normalize_signal(signal_raw, args.normalize_mode)
            ...
            for (l0, l1, r0, r1, mid_b0, mid_b1) in pairs:
                if written_regions >= args.max_regions:
                    break

                middle_len_bases = int(mid_b1 - mid_b0)

                if args.require_nonempty_middle and mid_b0 >= mid_b1:
                    continue

                if middle_len_bases < args.min_middle_len or middle_len_bases > args.max_middle_len:
                    continue

                if max_base_in_mv >= 0 and mid_b1 > (max_base_in_mv + 1):
                    seq_out_of_mv_cnt += 1
                    continue

                s0, s1 = base_to_step_bounds(step_base, mid_b0, mid_b1)
                if s0 is None:
                    continue

                mid_sig_from = int(s0 * args.stride)
                mid_sig_to = int(s1 * args.stride)

                mid_sig_from = max(0, min(mid_sig_from, len(signal)))
                mid_sig_to = max(0, min(mid_sig_to, len(signal)))
                if mid_sig_from >= mid_sig_to:
                    continue

                win_from = max(0, mid_sig_from - args.pad_samples)
                win_to = min(len(signal), mid_sig_to + args.pad_samples)
                if win_from >= win_to:
                    continue

                middle_len_bases = int(mid_b1 - mid_b0)
                middle_seq = seq_norm[mid_b0:mid_b1]

                base_sample_spans_rel, base_abs_spans, ok_any = build_base_sample_spans_rel(
                    step_base=step_base,
                    occ_start_base=mid_b0,
                    span_len_bases=middle_len_bases,
                    win_from=win_from,
                    win_to=win_to,
                    mv_stride=args.stride,
                    signal_len=len(signal),
                )
                if not ok_any and middle_len_bases > 0:
                    continue
                ...
                if args.unify_motif_orientation and is_reverse:
                    ...
                    middle_seq_out = middle_seq[::-1]

                txt_out.write(
                    f"{rid}\t{strand}\t{l0}\t{l1}\t{r0}\t{r1}\t{mid_b0}\t{mid_b1}\t{middle_len_bases}\t"
                    f"{mid_sig_from}\t{mid_sig_to}\t{win_from}\t{win_to}\t"
                    f"{left_norm}\t{right_norm}\t{left_to_search}\t{right_to_search}\n"
                )
                ...
                rec_json = {
                    ...
                    "strand": int(strand),
                    "left_flank_query": left_norm,
                    "right_flank_query": right_norm,
                    "left_flank_matched": left_to_search,
                    "right_flank_matched": right_to_search,
                    "orientation_unified": bool(args.unify_motif_orientation),
                    "left_start_base": int(l0),
                    "left_end_base": int(l1),
                    "right_start_base": int(r0),
                    "right_end_base": int(r1),
                    "middle_start_base": int(mid_b0),
                    "middle_end_base": int(mid_b1),
                    "middle_seq_len": int(middle_len_bases),
                    "middle_seq": middle_seq_out,
                    "mv_stride": args.stride,
                    "pad_samples": args.pad_samples,
                    "normalize_mode": args.normalize_mode,
                    "middle_signal_from": int(mid_sig_from),
                    "middle_signal_to": int(mid_sig_to),
                    "win_from": int(win_from),
                    "win_to": int(win_to),
                ...
                if not args.no_plot:
                    batch.append({
                        "rid": f"{rid} | middle={mid_b0}:{mid_b1}",
                    ...
                    if len(batch) >= args.batch_size:
                        batch_idx += 1
                        out_png = save_batch_png_subplots(batch, batch_idx, args.out_dir, args)
    ...
    qc = {
        "left_flank_input": args.left_flank,
        "right_flank_input": args.right_flank,
        "left_flank_normalized": left_norm,
        "right_flank_normalized": right_norm,
        "left_flank_reverse_complement": left_rc,
        "right_flank_reverse_complement": right_rc,
        "matching_mode": "ATGCN flank pairing",
        "pair_mode": args.pair_mode,
        "orientation_unified": bool(args.unify_motif_orientation),
        "scanned_lines": int(scanned_lines),
        "written_regions": int(written_regions),
        ...
        "no_valid_pair_cnt": int(no_valid_pair_cnt),
        ...
        "middle_len_bases_percentiles": safe_percentiles(middle_len_list),
        ...
        "notes": {
            ...
            "offset_check": "If seq_out_of_mv_cnt is high, seq base indices may not align with moves base indices.",
            "matching_rule": "Flanks support only A/T/G/C/N, where N matches any A/T/G/C.",
            "orientation_rule": "For reverse-strand matches, output window signal and per-base spans are reversed if --unify_motif_orientation is enabled.",
        }
    }
    ...
    print(f"[DONE] written regions (up to {args.max_regions}): {written_regions}")
...
def parse_args():
    ap = argparse.ArgumentParser(
        description=(
            "Extract the middle region between left/right flanks from nanopore reads, "
            "map it to raw signal using moves, optionally write window signal and plot."
        )
    )
    ...
    ap.add_argument("--left_flank", required=True, help="left fixed flank sequence, supports A/T/G/C/N")
    ap.add_argument("--right_flank", required=True, help="right fixed flank sequence, supports A/T/G/C/N")
    ap.add_argument(
        "--pair_mode",
        default="nearest",
        choices=["nearest", "all", "outer"],
        help="how to pair left/right flank hits"
    )

    ap.add_argument("--stride", type=int, default=5, help="raw samples per move step, often 5")
    ap.add_argument("--pad_samples", type=int, default=0, help="expand middle signal window on both sides")

    ap.add_argument("--max_regions", type=int, default=1000, help="maximum extracted regions to write/plot")
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="for reverse-strand matches, reverse middle window signal and per-base spans so all outputs are in the same orientation"
    )

    ap.add_argument("--require_nonempty_middle", action="store_true", help="skip cases where middle length is 0")
    ...
    ap.add_argument("--min_middle_len", type=int, default=120, help="minimum middle length in bases")
    ap.add_argument("--max_middle_len", type=int, default=150, help="maximum middle length in bases")
    ...
    ap.add_argument(
        ...
        nargs="*",
        default=[],
        help="highlight multiple middle-region base positions, 0-based"
    )
    ap.add_argument(
        "--max_bases_to_label",
        type=int,
        default=80,
        help="maximum number of middle bases to annotate on the plot"
    )
